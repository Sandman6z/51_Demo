...
void Rc522Init(void)
{
	...
	/* SPI clock rate 2MHz */
	SPI_SetClockFreq(SPI1, 1000000, 0);
...
//	unsigned char tmp;
    ...
    SET_RC522_RST();//MF522_RST=1;
    __nop();  __nop();__nop();__nop();__nop();
    CLS_RC522_RST();//MF522_RST=0;
    __nop(); __nop();__nop();__nop();__nop();
    SET_RC522_RST();//MF522_RST=1;
    __nop();__nop();__nop();__nop();__nop();
...
unsigned char ReadRawRC(unsigned char Address)
{
    ...
    temp = ((Address<<1)&0x7E) | 0x80;			/*   write Address */
...
{  
    ...
    temp = ((Address<<1)&0x7E);			/*   write Address */
    SPI_SingleWrite(SPI1, &temp);
	// wait
	while (SPI_IsBusy(SPI1)) {}
